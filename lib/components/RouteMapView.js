...
export default class RouteMapView extends Component {
  ...
  clippedStops() {
    if (!this.state.lastPosition) return this.props.stops;
  ...
  clippedCoordinates() {
    if (!this.state.lastPosition) return this.props.coordinates;
  ...
  zoomMap() {
    ...
    setTimeout(() => {
      ...
      let closest = this.closestStops(this.state.lastPosition, 4);
      closest.push(this.state.lastPosition.coords)
