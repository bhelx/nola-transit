...
export default class RouteMapView extends Component {
  ...
  closestStops(num) {
    return this.props.stops[1].slice(0, 3);
  }
  ...
  render() {
    ...
    let transparentColor = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.6)`;

    let disabledStops = clippedStops[0].map((stop, idx) => {
      let markerStyle = [styles.StopMarkerDisabled, {borderColor: this.props.color}];
    ...
    let stops = clippedStops[1].map((stop, idx) => {
      ...
      if (idx == 0) markerStyle = [styles.StopMarkerBig, {backgroundColor: this.props.color}];
