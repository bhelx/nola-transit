...
const gtfsIndex = new GtfsIndex();

export default class Home extends Component {
  constructor(props) {
    super(props);
    this.navIndex = NAV_STACK_INDEX;
    this.closestStopsData = null;
    this.state = {
      routeData: [],
      loading: true,
      ...
      .then(response => {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          e => {
            ...
            let position = {"coords": {"longitude": LONGITUDE, "latitude": LATITUDE}};
  ...
  updateInfo(position) {
    ...
    if (position && position.coords) {
      let routeCount = this.state.routeData.length;
      if (routeCount === 0) routeCount = ROUTE_PER_PAGE;
      gtfsIndex.getClosestEntryPoints(position.coords, (err, epIndex) => {
        Gtfs.updateHomePageInfo(epIndex, position.coords, routeCount, (err, routes) => {
  ...
  fetchNextPage() {
    ...
    if (position && position.coords) {
      ...
      let routeCount = this.state.routeData.length;
      gtfsIndex.getClosestEntryPoints(position.coords, (err, epIndex) => {
        Gtfs.getHomePageInfo(epIndex, position.coords, routeCount + 1, ROUTE_PER_PAGE, (err, routes) => {
          this.safelySetState({
            routeData: this.state.routeData.concat(routes),
            loadingNextPage: false,
          });
        });
      });
  ...
  renderListView() {
    if (this.state.loading) {
      ...
      let keyExtractor = (item, index) => (index + item.route_short_name);
      return (
        <FlatList
