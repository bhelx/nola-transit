...
const DATA_POLLING_INTERVAL = 60000; // 1 minute
const NAV_STACK_INDEX = 2; // TODO figure out a cleaner way to do this
...
export default class Stop extends Component {
  ...
  componentWillMount() {
    console.log('Stop will mount');
    this.props.navigation.removeListener(
      'change',
      this._handleNavStateChange.bind(this)
    );
  }
  componentDidMount() {
    console.log('Stop did mount');
    this.updateInfo();
    this.startListeners();
  }
  componentWillUnmount() {
    console.log('Stop will unmount');
    this.stopListeners();
  }
  _handleNavStateChange(state) {
    console.log('Stop focus state changed ', state);
    if (state.index != this.navIndex) {
      ...
      console.log('Stop new state doesnt concern me');
    }
  ...
  _handleAppStateChange(nextAppState) {
    if (
      ...
      console.log('Stop backrounded');
      this.stopListeners();
    }
    this.setState({ appState: nextAppState });
  }
  startListeners() {
    console.log('Stop start listeners');
    if (!this.dataWatchId) {
  ...
  stopListeners() {
    console.log('Stop stop listeners');
    clearInterval(this.dataWatchId);
    this.dataWatchId = null;
  }
  clearInfo() {
    console.log('Stop clear info');
    this.setState({
  ...
  updateInfo(unsafe) {
    console.log('Stop updating info');
  ...
  _hideModal() {
    this.safelySetState({ isReminderModalVisible: false });
  }

  onSchedulePress(scheduleItem) {
    this._showModal(scheduleItem);
  }
  scheduleNotification(routeType, time, mins) {
    let stop = this.props.navigation.state.params.stop;
    let route = this.props.navigation.state.params.route;
    let msg = `Your #${route.route_short_name} ${routeType} will be at ${stop.stop_name} in ${mins} minutes!`;
  ...
  onFavChange() {
    this.safelySetState({ favorited: !this.state.favorited });
  }
  ...
  renderRow(scheduleItem, secId, rowId) {
    let timeTil;
    let alarmIcon;
    let alarmCutoff = new Date();
    alarmCutoff.setSeconds(alarmCutoff.getSeconds() + 600); // 10 minute cutoff
    let showReminder = scheduleItem.datetime > alarmCutoff;
    ...
    return (
      <TouchableOpacity
        onPress={
          showReminder ? this.onSchedulePress.bind(this, scheduleItem) : null
        }
      >
