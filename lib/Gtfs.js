...
export default class Gtfs {
  ...
  // on the screen
  static updateHomePageInfo(myLoc, routeCount, done) {
    Gtfs.getHomePageInfo(myLoc, 0, routeCount, done);
  }

  // used to get a pageworth of data
  static getHomePageInfo(myLoc, routeIdx, pageSize, done) {
    const now = Gtfs.secsSinceMidnight();

    Gtfs.getServiceId((err, serviceId) => {
      if (err) return done(err);
      Gtfs.getRoutes(routeIdx, pageSize, (err, routes) => {
        if (err) return done(err);

        // let's only look at these routes right now for simplicity
        // let shortNames = ['11', '12', '10', '1', '16', '5'];
  ...
  static getRoutes(idx, perPage, done) {
    ...
    const sql = `
      SELECT
        ...
        route_text_color
      FROM routes
      WHERE route_index NOT IN ${filterRoutes}
