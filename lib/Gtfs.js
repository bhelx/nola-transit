...
export default class Gtfs {
    ...
    parallel(
      {
        4: callback => {
          parallel(
            [cb => Gtfs.getShape(7, cb), cb => Gtfs.getShape(8, cb)],
            callback
          );
        },
        5: callback => {
        ...
        1: callback => {
          parallel(
            [cb => Gtfs.getShape(128, cb), cb => Gtfs.getShape(129, cb)],
            callback
          );
        },
      },
      (err, results) => {
        ...
        lines.push({ color: 'ED99C2', coords: results[4][0] });
        lines.push({ color: 'ED99C2', coords: results[4][1] });
        lines.push({ color: '9B5AA5', coords: results[5][0] });
        lines.push({ color: '9B5AA5', coords: results[5][1] });
        lines.push({ color: '006737', coords: results[6][0] });
        lines.push({ color: '006737', coords: results[6][1] });
        done(null, lines);
      }
    );
  }
  ...
  static getSchedule(routeIndex, shapeIndex, serviceIds, stopIndex, now, done) {
    const sql = `
      SELECT
