...
export default class Gtfs {
  ...
  static getRouteInfo(routeIndex, myLoc, done) {
    console.log("getRouteInfo ", arguments)
    const sql = `
      SELECT
    ...
    Gtfs.executeSql(sql, (err, results) => {
      ...
      Gtfs.getClosestStops(routeIndex, [d0.direction_id, d1.direction_id], 1, myLoc, (err, closestStops) => {
        ...
        parallel({
          ...
          d0Stops: (callback) => {
            Gtfs.getStopsAndSchedule(routeIndex, d0.direction_id, 1, closestStops[0].stop_index, now, callback);
          },
          d1Stops: (callback) => {
            Gtfs.getStopsAndSchedule(routeIndex, d1.direction_id, 1, closestStops[1].stop_index, now, callback);
          },
        }, (err, results) => {
  ...
  static getStopsAndSchedule(routeIndex, directionId, serviceId, closestStopIndex, now, done) {
    const sql = `
      SELECT
      ...
      WHERE stop_times.trip_index = (
        ...
        WHERE trips.route_index = ${routeIndex} AND trips.direction_id = ${directionId} AND trips.service_id = ${serviceId}
  ...
  static getClosestStops(routeIndex, directionIds, serviceId, myLoc, done) {
    parallel([
      (callback) => {
        Gtfs.getClosestStop(routeIndex, directionIds[0], serviceId, myLoc, callback);
      },
      (callback) => {
        Gtfs.getClosestStop(routeIndex, directionIds[1], serviceId, myLoc, callback);
      },
    ], (err, results) => {
      if (err) return done(err);
      done(null, results)
