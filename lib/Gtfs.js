...
export default class Gtfs {
  ...
  static getRouteInfo(route_index, done) {
    const sql = `
      SELECT
    ...
    Gtfs.executeSql(sql, (err, results) => {
      ...
      parallel({
        ...
        d0_stops: (callback) => {
          Gtfs.getStops(route_index, d0.direction_id, callback);
        },
        d1_stops: (callback) => {
          Gtfs.getStops(route_index, d1.direction_id, callback);
        },
      }, (err, results) => {
  ...
  static getStops(route_index, direction_id, done) {
    const sql = `
      SELECT
