  ...
  static getHomePageInfo(myLoc, done) {
    ...
    Gtfs.getServiceIds((err, serviceIds) => {
      ...
      Gtfs.getRoutes((err, routes) => {
        ...
        let shortNames = ['11', '12', '10', '91', '5', '1'];
        routes = routes.filter(r => shortNames.indexOf(r.route_short_name) != -1);
          ...
          Gtfs.executeSql(sql, (err, results) => {
            ...
            Gtfs.getClosestStops(
              ...
              (err, closestStops) => {
                ...
                parallel(stopsFetchers, (err, stopResults) => {
                  if (err) return done(err);

                  let filteredDirections = directions.map((d, idx) => {
                    // we only one the closest stop which is now hydrated with the schedule
                    d.closest_stop = stopResults[idx].find(s => s.stop_index == closestStops[d.direction_id].stop_index);
                    return d;
                  });

                  routeInfoCb(null, directions: filteredDirections);
                });
              }
            );
          });
