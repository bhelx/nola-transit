...
export default class Gtfs {
      ...
      Gtfs.executeSql(sql, (err, results) => {
        ...
        Gtfs.getClosestStops(routeIndex, [d0, d1], serviceIds, myLoc, (err, closestStops) => {
          ...
          let stopsFetchers = directions.map(direction => {
            return (callback) => {
              Gtfs.getStopsAndSchedule(routeIndex, direction.trip_headsign, serviceIds, closestStops[direction.direction_id].stop_index, now, callback);
            }
          })
  ...
  static getStopsAndSchedule(routeIndex, tripHeadsign, serviceIds, closestStopIndex, now, done) {
    const sql = `
      SELECT
      ...
      WHERE stop_times.trip_index = (
        ...
        WHERE trips.route_index = ${routeIndex} AND trips.trip_headsign = "${tripHeadsign}" AND trips.service_id IN (${serviceIds.join()})
