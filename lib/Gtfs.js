...
export default class Gtfs {
  ...
  // Used to get the updated data for every route
  // on the screen
  static updateHomePageInfo(epIndex, myLoc, routeCount, done) {
    Gtfs.getHomePageInfo(epIndex, myLoc, 0, routeCount, done);
  }

  // used to get a pageworth of data
  static getHomePageInfo(epIndex, myLoc, routeIdx, pageSize, done) {
    const now = Gtfs.secsSinceMidnight();

    Gtfs.getServiceId((err, serviceId) => {
      if (err) return done(err);
      Gtfs.getRoutes(epIndex.routeList, routeIdx, pageSize, (err, routes) => {
        if (err) return done(err);

        let routeInfoFetcher = (route, routeInfoCb) => {
          let tripData = epIndex.index[route.route_index];
          let headsigns = Object.keys(tripData.headsigns);
          let stopsFetchers = headsigns.map(headsign => {
            let trip = tripData.headsigns[headsign];
            return callback => {
              Gtfs.getSchedule(
                route.route_index,
                trip.shape_index,
          ...
          parallel(stopsFetchers, (err, stopsResults) => {
            ...
            let filteredDirections = headsigns.map((h, idx) => {
              let trip = tripData.headsigns[h];
              // this gives us the closest stop but hydrated with the schedule information
              trip.closestStop = stopsResults[idx][0]; // it's the first result
              return trip;
            });
  ...
  static getRoutes(routeList, idx, perPage, done) {
    // filter out weird routes for now
    // TODO - why do some routes only have one direction id
    // and how do we handle that?
    filterRoutes = '(13, 35, 14, 36)';

    // this WITH clause makes a table that allows us to sort
    // by position in routeList
    const sql = `
      WITH route_order(pos,route_index) AS (
      VALUES
        ${routeList.map((r, i) => `(${i},${r})`).join(',')}
      )
      SELECT
        routes.route_index,
        ...
        route_order.pos AS position
      FROM routes
      JOIN route_order ON route_order.route_index = routes.route_index
      WHERE routes.route_index NOT IN ${filterRoutes}
      ORDER BY route_order.pos
      LIMIT ${idx}, ${perPage}
      ;
    `;
  ...
  /**
   * Gets all "entry points" onto the network. An
   * "entry point" is basically a unique tripple of
   * (route, stop, headsign).
   */
  static getAllEntryPoints(done) {
    const sql = `
      SELECT
        t.route_index,
        t.trip_headsign,
