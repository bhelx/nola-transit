...
export default class Route extends Component {
  constructor(props) {
    super(props);
    this.state = {
      destination: 'to_audobon',
    };
  }
  onChooseDesintation(destination) {
    this.setState({destination: destination});
  }
  render() {
    let destinationText = this.state.destination === 'to_audobon' ? 'Audobon Zoo' : 'Canal St';
    let coords = this.state.destination === 'to_audobon' ? toAudobon : toCanal;
      ...
      <View style={styles.Container}>
        <View style={[styles.Route, {backgroundColor: '#9B5AA5'}]}>
          <RouteShield
            ...
            number='11'
          />
          <Text style={[styles.RouteName, {color: '#fff'}]}>Magazine to {destinationText}</Text>
        </View>
        <MapView
        ...
        <View style={styles.DestinationChooser}>
          <Text>Direction:</Text>
          <Button
            onPress={this.onChooseDesintation.bind(this, 'to_audobon')}
            title="Audobon Zoo"
            color="#9B5AA5"
          />
          <Button
            onPress={this.onChooseDesintation.bind(this, 'to_canal')}
            title="Canal St"
            color="#9B5AA5"
          />
        </View>
