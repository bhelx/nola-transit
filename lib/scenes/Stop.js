...
export default class Stop extends Component {
  constructor(props) {
    ...
    this.state = {
      dataSource: ds.cloneWithRows(this.annotateSchedule([])),
      dayOfWeek: 'Sat',
      timeSelected: 3,
      favorited: false
    }
  ...
  annotateSchedule(schedule) {
    let currentTime = this.getCurrentTime();
    let nexts = 0;
    return schedule.map(s => {
      let scheduleTime = s.departure_time;
      if (scheduleTime.length == 7) scheduleTime = "0" + scheduleTime;

      if (nexts < 2 && scheduleTime > currentTime) {
        nexts++;
        return {time: s.departure_time, next: true, timeAway: "X mins."}
      }
      return s;
    });
  }
