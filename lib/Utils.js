/**
 * Junk draw namespace for utility functions.
 *
 * @flow
 */
...
export default class Utils {
  ...
  /**
   * Gets the current datetime offset by
   * config value.
   */
  static getCurrentDateTime() {
    let d = new Date();
    // offset the time by config value (default 0)
    d.setSeconds(d.getSeconds() + Config.clockOffset || 0);
    return d;
  }
  ...
  /**
   * Turns seconds into a data structure to humanize the results:
   * example:
   *
   * > toTimeAway(100000)
  ...
  static secsToDateTime(secs) {
    let midnight = Utils.getCurrentDateTime();
  ...
  static colorForRoute(route) {
    return Utils.routeColor(route.route_color);
  }

  /**
   * Turns a stop name like "Washington at Magazine" to "Washington / Magazine"
   */
  static slashedName(name) {
